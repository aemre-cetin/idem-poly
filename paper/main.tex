...
\author{\IEEEauthorblockN{Dr. A. Emre \c{C}etin}
\IEEEauthorblockA{Independent Research, Istanbul, Turkey\\
...
\end{equation}
and feed-forward projections with 3D Chebyshev tensor contractions. Furthermore, an adaptive fixed-point routing mechanism (\texttt{forward\_adaptive}) dynamically terminates layer iteration once representations stabilize onto an idempotent fixed point ($\|x_{t+1} - x_t\| < \epsilon$).

\subsection{Empirical Comparison: Standard Transformer vs. PolyFormer}
We evaluate a 4-layer PolyFormer-Tiny ($d_{\text{model}}=128$, 4 heads, Chebyshev degree $K=3$) head-to-head against an architecturally identical Standard Transformer baseline (Multi-Head Attention + 4x SwiGLU MLP). Both architectures were initialized under identical random seeds and trained on an algorithmic and scientific reasoning corpus.

\begin{table}[h]
\centering
\caption{Ground-Up Training \& Inference Benchmark}
\label{tab:polyformer_benchmark}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lccc}
\toprule
\textbf{Metric} & \textbf{Standard Transformer} & \textbf{PolyFormer-Tiny (Ours)} & \textbf{Advantage} \\
\midrule
Total Parameters & 1,132,800 ($\sim$1.13M) & \textbf{609,036 ($\sim$609K)} & \textbf{-46.2\% Parameters} \\
FP32 Model Size & 4.32 MB & \textbf{2.32 MB} & \textbf{-46.3\% Static VRAM} \\
FFN Parameters / Layer & 393,216 & \textbf{65,536} & \textbf{-83.3\% FFN Params} \\
...
Mean Output Cosine Alignment & 100.0\% (Reference) & \textbf{93.06\%} & High Semantic Fidelity \\
\bottomrule
\end{tabular}%
}
\end{table}

As shown in Table~\ref{tab:polyformer_benchmark}, PolyFormer cuts overall model parameters by \textbf{-46.2\%} and static memory by \textbf{-46.3\%}, while sustaining a competitive real-time inference speed of 202.7~tokens/s on CPU.

\subsection{Output Fidelity and Representation Alignment}
To evaluate whether the 46.2\% parameter reduction causes representational divergence, both models were probed with identical test prompts. Next-token logit distributions achieved \textbf{90.19\% to 95.10\% cosine similarity} (mean 93.06\%), proving that orthogonal polynomial contractions faithfully reproduce the latent geometric manifold of standard deep architectures.

\subsection{Asymptotic Convergence and Compute-Optimal Scaling}
A critical theoretical question in compact non-linear neural design is whether parameter-reduced architectures encounter a fundamental capacity ceiling, or whether additional training iterations achieve asymptotic equivalence with larger baselines. To investigate this, we tracked PolyFormer's convergence trajectory across extended optimization horizons:
\begin{itemize}
...
To facilitate community adoption and independent verification, all components of this research have been released as public artifacts:
\begin{itemize}
    \item \textbf{PyPI Library:} \texttt{idempotent-poly} (v0.1.1, installable via \texttt{pip install idempotent-poly}).
    \item \textbf{Surgery Model Checkpoint:} \texttt{aecetin/SmolLM2-135M-PolyFFN} on Hugging Face Model Hub (39.76M parameter savings).
    \item \textbf{From-Scratch Model Checkpoint:} \texttt{aecetin/PolyFormer-Tiny} on Hugging Face Model Hub (609K parameters, native \texttt{AutoModelForCausalLM} support).
